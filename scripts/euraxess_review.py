...
from euraxess_adapter import discover_offer_urls, fetch_offer_title, fetch_search_html
    ...
    try:
        for url in urls:
            try:
                ...
                continue
            added = add_candidate(
                db, source_id="euraxess-jobs", source_url=url, original_title=title
            ) if db else None
            results.append({"source_url": url, "original_title": title, "added_to_review": added})
    finally:
        if db:
            db.close()
    return results
