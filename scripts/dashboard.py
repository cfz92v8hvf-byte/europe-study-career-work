...
<h1>Европа | Учёба • Карьера • Работа</h1><p class=note>Панель очереди. Автопубликация отключена до отдельной проверки.</p><h2>Статусы</h2><ul>{cards}</ul><h2>Готовность карточек</h2><p>Полностью переведено для редакционной проверки: {ready}; без полного перевода: {incomplete}.</p><h2>Перевод</h2><p>{translation_note}</p><h2>Официальные источники</h2><p>{source_note}</p><p>Обновлено (UTC): {datetime.now(timezone.utc).isoformat()}</p>""", encoding="utf-8")
