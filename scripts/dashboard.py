...
<h1>Европа | Учёба • Карьера • Работа</h1><p class=note>Панель очереди. Автопубликация отключена до отдельной проверки.</p><h2>Статусы</h2><ul>{cards}</ul><h2>Официальные источники</h2><p>{source_note}</p><p>Обновлено (UTC): {datetime.now(timezone.utc).isoformat()}</p>""", encoding="utf-8")
