...
from euraxess_adapter import discover_offer_urls, extract_offer_summary, extract_offer_title
...
class EuraxessAdapterTests(unittest.TestCase):
    ...
    def test_extracts_only_a_substantial_official_description(self):
        html = '<meta property="og:description" content="Research position in a European laboratory with a multidisciplinary team and funded mobility support.">'
        self.assertEqual(extract_offer_summary(html), "Research position in a European laboratory with a multidisciplinary team and funded mobility support.")
        self.assertIsNone(extract_offer_summary('<meta name="description" content="Too short">'))
